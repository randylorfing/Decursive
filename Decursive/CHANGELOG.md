...
### Alert warnings
- **DISPEL alert warning** — center banner when a MUF shows a dispellable affliction (separate from chat notifications). Options: enable/disable, **Timed** (default 3s) vs **Until cleared**, duration slider, test buttons, font size (default 48, range 16–96) and color. Draggable anchor via `/dcralerts move`.
- **Fix:** Timed-fade work broke live DISPEL text by forcing layer alpha to 0 every refresh tick and on layer OnHide, overriding the working AuraSlot Show/Hide cascade. Restored cascade-driven visibility; duration hide is owned by `OnShow` + `armDispelLayerTimedHide` only.

### Micro Unit Frames
- **MUF size controls** — Party and Raid size sliders (10–80 px) added to Modern UI (Settings → Micro Unit Frames → Layout & Display). Resize APIs existed but were not wired to any control.
- **Status indicator light toggle** — new Display option; **default OFF**. When off, lights are hidden and party/raid MUF spacing restores to pre-status-light layout.

### Debuff identity tooltip (dungeon/raid)
- **Fix:** Native identity `AuraContainer` was missing `SetUnit`, `SetEnabled`, `Show`, and `UpdateAllAuras`; Blizzard's built-in debuff tooltip never activated. Unit retarget now refreshes the identity container.
- **Unit name label** restored on MUF hover — class-colored name anchored above/below the square alongside the native debuff tooltip (replaces generic affliction tooltip body when identity slot is active).
- **PvP/arena:** debuff identity tooltip (native slot + cast-inference fallback) fully disabled.

### Options UI
- Priority & Skip list reorder buttons: fixed mojibake; labels are now plain **Up / Dn / Top / End / X**.
