...
-- to settle on a transitional snapshot. A single roster snapshot taken during
-- that transition may therefore be incomplete even though the final party is
-- still intact. Re-scan a few times after the spec swap and always rebuild from
-- Blizzard's current roster. This is deliberately a bounded recovery sequence
-- rather than a permanent polling loop.
...
function D:SchedulePostSpecializationMUFRecovery()
    ...
    -- Protect the current known-good follower/party roster while Blizzard
    -- rebuilds role/spec state. A PLAYER_ROLES_ASSIGNED nudge may call this
    -- again and naturally extend the protection window.
...
do
    ...
    function D:GetUnitArray() --{{{
        ...
        -- last stable party snapshot until Blizzard reports the full party again.
        if postSpecRosterRecoveryActive() and LAST_STABLE_PARTY_ROSTER
