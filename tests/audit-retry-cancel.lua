...
engine:RegisterConsumer("Alerts", function()
  return not fail, fail and "FAILURE" or "SUCCESS", 0
end)
Check(not engine:Refresh("FIRST_ALERTS_FAILURE"), "first failure rejected")
Check(engine.retryScheduled, "first failure has a retry")
local recoveryCallback = timerQueue[#timerQueue]
Check(not engine:Refresh("SPELLS_CHANGED_WHILE_RECOVERING"), "second failure rejected")
fail = false
recoveryCallback()
io.write("after second failure and old timer: state=" .. engine.state ..
  " pending=" .. tostring(engine.pending) .. " scheduled=" .. tostring(engine.retryScheduled) ..
  " attempts=" .. engine.retryAttempts .. " availableTimers=" .. #timerQueue .. "\n")
Check(engine.state == "READY" or engine.retryScheduled, "recovery must complete or retain a live retry after the failure clears")
