...
local engine, owner, setUnit = Fixture()
local old = engine.carrierByOwner[owner]
Check(old.container.enabled and old.container.shown, "previous native carrier working")
setUnit("party1")
engine:RegisterConsumer("Alerts", function() return false, "DEFERRED_RESTRICTED", 0 end)
local ok, status = engine:Refresh("ROSTER_CHANGE_WITH_AUDIO_LOCKED")
local record = engine.carrierByOwner[owner]
io.write("audio deferred: state=" .. engine.state .. " nativeUnit=" .. record.unit ..
  " enabled=" .. tostring(record.container.enabled) .. " shown=" .. tostring(record.container.shown) ..
  " active=" .. tostring(record.active) .. " configured=" .. record.configuredGeneration .. "\n")
Check(not ok and status == "DEFERRED_RESTRICTED", "global transaction correctly remains uncommitted")
Check(record.container.enabled == true, "audio-only deferral must preserve or complete the independent native MUF provider")
