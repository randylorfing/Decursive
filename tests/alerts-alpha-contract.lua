...
Check(alerts:find('ShowSizedAlertText("Dispelled", pack)', 1, true), "successful dispel text has the exact requested content")
Check(mufs:find("ns.ConfigureDispelAlertSlot(slot)", 1, true), "MUF native slots register alert presentation")
Check(mufs:find("ShouldBeginSoulLinkAttempt(self, button)", 1, true), "MUF pre-click gates the actual installed action")
Check(mufs:find("not IsPubliclyDeadUnit(btn.unit)", 1, true), "living dispel cannot arm battle-rez warning")
Check(mufs:find("ClickRowForButton(btn.soulLinkRows, button)", 1, true), "Soul Link uses installed gesture metadata")
Check(mufs:find("ns.BeginSoulLinkAttempt(nil)", 1, true), "every MUF click clears stale Soul Link attribution")
