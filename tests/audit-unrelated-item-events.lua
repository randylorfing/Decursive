...
local before = engine.refreshGeneration
for i = 1, 100 do
  local beforeTimers = #timerQueue
  engine:OnEvent("GET_ITEM_INFO_RECEIVED", 10000 + i, true)
  if #timerQueue > beforeTimers then timerQueue[#timerQueue]() end
end
io.write("100 unrelated successful item loads cause " .. (engine.refreshGeneration - before) .. " complete reconciliations\n")
Check(engine.refreshGeneration == before, "unrelated item data must not rebuild dispel providers")
