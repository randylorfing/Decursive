...
Check(options:find('label = "Out of range", description = "RGB color only. Out-of-range dim controls brightness exactly once.", kind = "color", hasOpacity = false', 1, true), "range picker is explicitly RGB-only")
Check(options:find('["color|Out of range"] = {group = "Squares", simple = true}', 1, true), "Simple Color exposes the range picker")
...
Check(options:find("if key == env then", 1, true), "editing indication remains independent from the applied icon")
Check(options:find('chip:SetScript("OnClick", function()', 1, true), "plain top chips retain click handlers")
Check(options:find('Addon():SetEditingEnvironment(row.key)', 1, true), "top chip clicks still select the editing environment")
Check(options:find('Addon():SetEnvironmentMode("multiple")', 1, true), "Environment Profiles exposes Multiple mode")
Check(options:find('Addon():SetEnvironmentMode("solo")', 1, true), "Environment Profiles exposes Solo mode")
Check(not options:find("OpenStaticEnvironmentMenu", 1, true), "Environment Profiles removes the obsolete static selector")
Check(options:find("ui.simple = not ui.simple", 1, true), "Simple and full mode toggle remains available")
...
Check(core:find('if ns.RefreshOptions then', 1, true), "environment transitions refresh the visible chip and pending state")
