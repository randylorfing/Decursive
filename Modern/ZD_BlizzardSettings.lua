...
-- Blizzard Settings -> AddOns integration.
-- This is intentionally a launcher/status surface only. The resizable v11 UI
-- remains the single source of truth for configuration.
...
local function getProviderName()
    if ZD.GetDandersFramesIntegrationStatus then
        local st = ZD:GetDandersFramesIntegrationStatus() or {}
        return st.displayName or (st.sessionProvider == "DANDERSFRAMES" and "DandersFrames" or "Native Blizzard-managed")
    end
...
function ZD:RefreshBlizzardSettingsPanel()
    ...
    local status = self.GetDandersFramesIntegrationStatus and self:GetDandersFramesIntegrationStatus() or nil
    if status and status.sessionProvider == "DANDERSFRAMES" then
        if status.active then
            panel.providerNote:SetText("DandersFrames was detected automatically and supplies protected dispel detection plus range/order data. Decursive owns MUFs, secure curing, cooldowns and status feedback.")
            setTextColor(panel.providerNote, 0.45, 0.95, 0.60)
        else
            panel.providerNote:SetText("DandersFrames was detected but its provider is not currently operational. Reload after updating/enabling DandersFrames; otherwise Decursive uses Native detection on the next startup when DandersFrames is unavailable.")
            setTextColor(panel.providerNote, 1.0, 0.45, 0.35)
        end
    else
...
function ZD:RegisterBlizzardSettingsPanel()
    ...
    makeButton(panel, "Integrations", 145, 266, -390, function()
...
-- Register once the addon has loaded. The panel's status values are refreshed
-- lazily on show so it remains accurate after profile/provider/environment changes.
if not ZD:RegisterBlizzardSettingsPanel() then
    ...
    registrationFrame:SetScript("OnEvent", function(self)
        if ZD:RegisterBlizzardSettingsPanel() then
