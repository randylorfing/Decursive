-- Zhaohu's Decursive v11 modern configuration UI.
local addonName, T = ...
local D = T and T.Dcr
local DC = T and T._C
...
local SEARCH_NATIVE_ENTRIES = {
    dashboard = {
        ...
        { "Test MUFs", "micro unit frames diagnostics" },
    },
    frames = {
    ...
    profiles = {
        ...
        { "Environment Mode", "Automatic Open World Dungeon Follower Mythic+ Raid PvP DandersFrames provider profile" },
    ...
    integrations = {
        { "DandersFrames integration", "DandersFrames Dander Frames integration toggle enable disable detection provider" },
        { "DandersFrames version", "integration API status" },
        { "Detection provider", "DandersFrames native Decursive source of truth" },
        { "Reload UI", "apply detection provider integration" },
    },
    compat121 = {
...
function ZD:CreateUI()
    ...
    savedW = math.max(960, math.min(savedW or 1040, 1500))
    savedH = math.max(650, math.min(savedH or 760, 1000))
    ...
    f:SetMovable(true)
    f:SetResizable(true)
    if f.SetResizeBounds then f:SetResizeBounds(960, 650, 1500, 1000) else
        if f.SetMinResize then f:SetMinResize(960,650) end
        if f.SetMaxResize then f:SetMaxResize(1500,1000) end
    end
    f:EnableMouse(true)
    ...
    -- Treat the standalone v11 settings window like a normal Escape-closeable
    -- Blizzard dialog. This keeps Escape behavior predictable regardless of
    -- whether the window was opened by slash command or from Settings > AddOns.
    if UISpecialFrames then
        local registered = false
        for _, frameName in ipairs(UISpecialFrames) do
            if frameName == f:GetName() then
                registered = true
                break
            end
        end
    ...
    header:SetHeight(72)
    makeBackdrop(header, C.panel, C.border)
    header:EnableMouse(true)
    header:RegisterForDrag("LeftButton")
    header:SetScript("OnDragStart", function() if not InCombatLockdown() then f:StartMoving() end end)
    header:SetScript("OnDragStop", function() f:StopMovingOrSizing() end)

    local icon = header:CreateTexture(nil, "ARTWORK")
    icon:SetTexture(DC.IconON)
    icon:SetSize(38, 38)
    icon:SetPoint("LEFT", 18, 0)

    header.title = label(header, "Zhaohu's Decursive", 20, C.text, "TOPLEFT", 70, -13)
    header.subtitle = label(header, "Detect • Cleanse • Protect • WoW Retail 12.1", 11, C.muted, "TOPLEFT", 70, -42)
    header.version = label(header, self.version, 11, C.accent, "TOPRIGHT", -48, -18)

    -- Global settings search. Results are shown as a lightweight dropdown so
    -- the sidebar remains dedicated to navigation.
    local searchBox = editBox(header, 286, 30, false)
    searchBox:SetPoint("TOPRIGHT", -98, -36)
    ...
    for i = 1, 8 do
        ...
        row:SetPoint("TOPLEFT", 5, -5 - ((i - 1) * 44))
        row:SetPoint("TOPRIGHT", -5, -5 - ((i - 1) * 44))
        row:SetHeight(40)
        makeBackdrop(row, C.card, C.border)
        row.title = label(row, "", 11, C.text, "TOPLEFT", 10, -6)
        row.title:SetPoint("RIGHT", -10, 0)
        row.title:SetJustifyH("LEFT")
        row.path = label(row, "", 9, C.muted, "BOTTOMLEFT", 10, 6)
    ...
    searchBox.edit:SetScript("OnEditFocusLost", function(self)
        if self:GetText() == "" then searchBox.placeholder:Show() end
        C_Timer.After(.10, function()
            if f.searchResults and not f.searchResults:IsMouseOver() and not self:HasFocus() then
    ...
    status:SetHeight(36)
    makeBackdrop(status, C.panel, C.border)
    status.text = label(status, "Ready", 11, C.muted, "LEFT", 16, 0)
    status.combat = label(status, "", 11, C.danger, "RIGHT", -16, 0)
    f.status = status
    ...
    navScroll:SetPoint("TOPLEFT", 8, -8)
    navScroll:SetPoint("BOTTOMRIGHT", -28, 8)
    local navChild = CreateFrame("Frame", nil, navScroll)
    navChild:SetWidth(174)
    ...
    for _, group in ipairs(navGroups) do
        local head = label(navChild, group.title, 9, C.muted, "TOPLEFT", 8, y)
        head:SetWidth(158); head:SetJustifyH("LEFT")
        y = y - 22
        for _, item in ipairs(group.items) do
            local key, name = item[1], item[2]
            local b = button(navChild, name, 166, 27, nil)
            b._pageKey = key
            b:SetScript("OnClick", function(btn) self:ShowPage(btn._pageKey) end)
            b:SetPoint("TOPLEFT", 4, y)
            b.text:SetJustifyH("LEFT")
            b.text:ClearAllPoints(); b.text:SetPoint("LEFT", 11, 0)
    ...
    -- Bottom-right resize grip. The saved size is per user profile and is
    -- restored on the next UI open/reload.
...
function ZD:PageTitle(page, title, subtitle)
    page.pageTitle = label(page, title, 20, C.text, "TOPLEFT", 0, 0)
    page.pageSubtitle = label(page, subtitle or "", 11, C.muted, "TOPLEFT", 0, -30)
...
function ZD:BuildDashboard(parent)
    local p = pageFrame(parent)
    self:PageTitle(p, "Dashboard", "One modern settings system for the complete Decursive feature set.")
    ...
    for i = 1, 3 do
        ...
        c:SetSize(cardW, 110)
        c:SetPoint("TOPLEFT", (i - 1) * (cardW + 12), -66)
        makeBackdrop(c, C.card, C.border)
        c.caption = label(c, "", 10, C.muted, "TOPLEFT", 14, -14)
        c.value = label(c, "", 15, C.text, "TOPLEFT", 14, -38)
        c.detail = label(c, "", 10, C.muted, "TOPLEFT", 14, -68)
        cards[i] = c
    end
    p.cards = cards

    local architecture = section(p, "v11 Single-UI Architecture", -194, 164)
    local lines = {
        "• WoW 12.1 detection always stays on Blizzard-managed protected aura carriers.",
        "• If DandersFrames is available, Decursive automatically uses it as the dispel provider and mirrors its unit-frame order.",
        "• Secure MUF click-casting and all existing behavior remain intact underneath.",
        "• If DandersFrames is not available, Decursive automatically falls back to native Blizzard-managed detection.",
        "• The legacy AceConfig settings window is no longer registered or exposed.",
    }
    ...
    local b1 = button(actions, "Test MUFs", 150, 30, function()
    ...
    function p:Refresh()
        ...
        if ZD:IsDandersFramesSessionSelected() then
            cards[2].detail:SetText("Single integration behavior profile")
        else
...
function ZD:BuildFrames(parent)
    local skipDisplay = {
        ...
        CooldownPriority2Pulse121Enabled=true, Test121MUFSelect=true,
        Test121MUFVisualsOne=true, Test121MUFVisualsAll=true,
    }
    local p = self:BuildTabbedOptionPathPage(parent, "Micro Unit Frames",
        "MUF settings are separated by purpose. Party and Raid can use different MUF sizes, large raids can auto-reflow into a compact grid, and every MUF has its own status light above the square: gray ready, yellow range warning, red failed/uncleared cure, green successful cleanse for 3 seconds.", {
    ...
    local basics = section(p, "Frame Basics", -146, 104)
...
function ZD:BuildCooldowns(parent)
    ...
    local envCycleValues=function()
        if ZD:IsDandersFramesSessionSelected() then return {{key="DANDERSFRAMES",name="DandersFrames"}} end
        local out={}; for _,key in ipairs(ZD.environmentOrder) do out[#out+1]={key=key,name=ZD.environmentNames[key]} end; return out
    end
...
function ZD:BuildProfiles(parent)
    local p = pageFrame(parent)
    self:PageTitle(p, "Profiles & Modes", "User profiles store your setup; environment modes decide which behavior block is active.")
    ...
    p.profileCycle = cycleButton(p, "User profile", -68, profileValues,
        function() return ZD:GetUserProfileName() end,
        function(name) ZD:SetUserProfile(name) end)

    local user = section(p, "Profile Management", -112, 172)
    ...
    local modeValues = function()
        if ZD:IsDandersFramesSessionSelected() then
            return { { key = "DANDERSFRAMES", name = "DandersFrames" } }
        end
        return {
    ...
    local behavior = section(p, "Environment Mode", -298, 176)
    ...
    function p:Refresh()
        ...
        if ZD:IsDandersFramesSessionSelected() then
            p.modeHelp:SetText("DandersFrames was selected automatically. One DandersFrames behavior profile is used everywhere; Open World, Dungeon, Mythic+, Raid and PvP profile switching is paused.")
        else
...
function ZD:BuildLists(parent)
    ...
    local function rebuildCard(card)
        ...
        for i, value in ipairs(list) do
            ...
            nm:SetWidth(128); nm:SetJustifyH("LEFT")
            local up = button(row, "↑", 22, 24, function() move(card.kind, i, "up") end); up:SetPoint("LEFT", 134, 0)
            local down = button(row, "↓", 22, 24, function() move(card.kind, i, "down") end); down:SetPoint("LEFT", 158, 0)
            local top = button(row, "⇧", 22, 24, function() move(card.kind, i, "top") end); top:SetPoint("LEFT", 182, 0)
            local bottom = button(row, "⇩", 22, 24, function() move(card.kind, i, "bottom") end); bottom:SetPoint("LEFT", 206, 0)
            local remove = button(row, "×", 22, 24, function()
                if not ZD:CanConfigure() then return end
                if card.kind == "priority" then D:RemoveIDFromPriorityList(i) else D:RemoveIDFromSkipList(i) end
                ZD:SetStatus("List entry removed.")
                p:Refresh()
            end, "danger"); remove:SetPoint("LEFT", 234, 0)
...
function ZD:BuildIntegrations(parent)
    local p = pageFrame(parent)
    self:PageTitle(p, "Integrations", "Choose whether DandersFrames or Decursive's native Blizzard-managed provider supplies dispel detection.")

    local dfCard = section(p, "DandersFrames Integration", -66, 286)
    p.dfToggle = switch(dfCard, "Enable DandersFrames integration", -40,
        function() return ZD:GetDandersFramesIntegrationEnabled() end,
        function(v) ZD:SetDandersFramesIntegrationEnabled(v) end,
        "Enabled by default when DandersFrames is detected on first use. Your explicit choice is saved. Changing providers requires a UI reload.")

    p.dfProvider = label(dfCard, "", 12, C.text, "TOPLEFT", 18, -116)
    p.dfVersion = label(dfCard, "", 11, C.muted, "TOPLEFT", 18, -144)
    p.dfState = label(dfCard, "", 11, C.muted, "TOPLEFT", 18, -174)
    p.dfState:SetPoint("RIGHT", -18, 0)
    p.dfState:SetJustifyH("LEFT")

    local reload = button(dfCard, "Reload UI", 120, 30, function() ReloadUI() end, "primary")
    reload:SetPoint("BOTTOMRIGHT", -18, 18)
    p.reloadButton = reload

    local contract = section(p, "Integration Contract", -368, 196)
    local contractLines = {
        "DandersFrames supplies: whether this unit has a matching dispel need, filtered by cure priority.",
        "Zhaohu's Decursive keeps: MUFs, cure spell mapping, mouse bindings, secure clicks and target handling.",
        "Zhaohu's Decursive keeps: priority rules, cooldown overlays/timers, secure curing, and the dedicated DandersFrames behavior profile.",
        "Status lights: with DandersFrames active, yellow mirrors DandersFrames' live range result. A cure attempt that fails is red. After a successful cast, a protected DandersFrames verification carrier checks whether the matching affliction remains: remaining = red; cleared = green for 3 seconds. LoS is still learned from an actual failed cleanse because DandersFrames has no continuous public LoS result.",
        "DandersFrames unit-frame ORDER is automatically mirrored by the Decursive MUFs whenever DandersFrames is the detected provider. Styling and click-casting remain independent.",
        "If the integration toggle is off, or DandersFrames is not installed/available, Decursive uses its native Blizzard-managed provider.",
    }
    ...
    function p:Refresh()
        local st = ZD:GetDandersFramesIntegrationStatus() or {}
        p.dfProvider:SetText("Session detection provider: " .. tostring(st.displayName or st.sessionProvider or "Unknown"))
        if st.active then
            p.dfProvider:SetTextColor(unpack(C.accent))
        elseif st.sessionProvider == "DANDERSFRAMES" then
            p.dfProvider:SetTextColor(unpack(C.danger))
        else
            p.dfProvider:SetTextColor(unpack(C.text))
        end
        p.dfVersion:SetText("DandersFrames version: " .. tostring(st.version or (st.available and "detected" or "not available")))
        if p.dfToggle and p.dfToggle.control then
            p.dfToggle.control:SetEnabled(st.available == true)
            p.dfToggle.control:SetAlpha(st.available and 1 or .45)
            p.dfToggle.control:Refresh()
        end
        if st.reloadRequired then
            p.dfState:SetText("Preference changed. Reload the UI to switch to " .. (st.configuredEnabled and "DandersFrames" or "Native Blizzard-managed") .. " detection.")
            p.dfState:SetTextColor(unpack(C.accent))
            p.reloadButton:Show()
        elseif st.sessionProvider == "DANDERSFRAMES" and not st.active then
            p.dfState:SetText("DandersFrames was detected but its provider is not operational. " .. tostring(st.reason or ""))
            p.dfState:SetTextColor(unpack(C.danger))
            p.reloadButton:Show()
        elseif st.active then
            if st.operational then
                local order = D.GetDandersFramesOrderSyncStatus and D:GetDandersFramesOrderSyncStatus() or nil
                local orderText = order and (" MUF order mirror: " .. tostring(order.method or "waiting") .. ", " .. tostring(order.matched or 0) .. " matched.") or ""
                p.dfState:SetText("Active. DandersFrames supplies dispel detection and unit-frame ordering; Decursive owns curing/cooldowns and uses one DandersFrames behavior profile everywhere." .. orderText)
            else
                p.dfState:SetText("Ready. DandersFrames API is available; protected detection carriers initialize with the MUFs.")
            end
            p.dfState:SetTextColor(unpack(C.accent))
            p.reloadButton:Hide()
        else
            if st.available and not st.configuredEnabled then
                p.dfState:SetText("DandersFrames is available but integration is disabled by your preference. Native Blizzard-managed detection is active.")
            else
                p.dfState:SetText("DandersFrames is not available. Native Blizzard-managed detection is active.")
            end
            p.dfState:SetTextColor(unpack(C.muted))
            p.reloadButton:Hide()
        end
    end
    p:Refresh()
...
function ZD:BuildSounds(parent)
    ...
    local rule = label(behavior,
        "12.1 engine: known dispellable Spell ID applied → Blizzard plays the selected alert.\nThe protected red/blue square remains independently managed by Blizzard.",
    ...
    function p:Refresh()
        ...
        if active then
            p.triggerState:SetText("Trigger engine: Active — MUF-linked affliction monitoring")
...
function ZD:BuildMessages(parent)
    return self:BuildOptionGroupPage(parent, "MessageOptions", "Messages", "Chat, error and notification behavior.")
end
