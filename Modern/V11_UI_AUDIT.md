...
| Integrations | Keep dedicated; DandersFrames contract remains explicit. |
...
- Grouped, scrollable sidebar prevents clipping as pages are added.
- Content remains scrollable independently from navigation.

## DandersFrames boundary

DandersFrames integration remains detection-only. It can decide which protected dispel carrier is active, but it does not own Decursive UI, cooldowns, range feedback, line-of-sight feedback, secure clicks, priorities or profiles.


### Alpha 15 integration note
When DandersFrames integration is active, MUF unit order mirrors DandersFrames using its public frame lookup and OnFramesSorted callback.
