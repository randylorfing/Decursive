...
function ZD:GetDandersFramesIntegrationEnabled()
    return D.profile and D.profile.DandersFramesDispelIntegrationEnabled == true or false
end

function ZD:SetDandersFramesIntegrationEnabled(enabled)
    if not self:CanConfigure() then return false end
    if not D.profile then return false end

    local status = D.Get121DispelDetectionProviderStatus and D:Get121DispelDetectionProviderStatus() or nil
    if enabled and status and not status.available then
        self:SetStatus("DandersFrames is not currently available. Install/enable DandersFrames, then reload the UI.", true)
        return false
    end

    D.profile.DandersFramesDispelIntegrationEnabled = enabled and true or false
    D.profile.DandersFramesDispelIntegrationUserSet = true

    local selected = status and status.sessionProvider == "DANDERSFRAMES" or false
    if selected ~= (enabled and true or false) then
        self:SetStatus("DandersFrames integration preference saved. Reload the UI to switch the protected detection provider.")
    else
        self:SetStatus(enabled and "DandersFrames integration is enabled." or "DandersFrames integration is disabled; Native Blizzard-managed detection is active.")
    end
    return true
end

function ZD:GetDandersFramesIntegrationStatus()
    if D.Get121DispelDetectionProviderStatus then
        return D:Get121DispelDetectionProviderStatus()
    end
    return {
        configuredEnabled = self:GetDandersFramesIntegrationEnabled(),
        sessionProvider = "NATIVE",
        available = false,
        active = false,
        reloadRequired = false,
        reason = "12.1 provider status is not available yet.",
    }
end

function ZD:IsDandersFramesSessionSelected()
    local status = self:GetDandersFramesIntegrationStatus()
    return status and status.sessionProvider == "DANDERSFRAMES" or false
end
...
function ZD:GetEnvironmentSetting()
    if self:IsDandersFramesSessionSelected() then return "DANDERSFRAMES" end
...
function ZD:SetEnvironmentSetting(mode)
    if not self:CanConfigure() then return false end
    if self:IsDandersFramesSessionSelected() then
        self:SetStatus("DandersFrames was selected automatically. Open World / Dungeon / Mythic+ / Raid / PvP behavior switching is paused while the DandersFrames provider is active.")
        return false
    end
...
function ZD:GetEditEnvironment()
    ...
    if not ENV_NAMES[key] or key == "DANDERSFRAMES" then key = "OPEN_WORLD" end
    return key
end

function ZD:SetEditEnvironment(key)
    if self:IsDandersFramesSessionSelected() then
        self.editEnvironment = "DANDERSFRAMES"
        self:SetStatus("Editing DandersFrames behavior. Environment-specific behavior switching is paused while the automatically selected DandersFrames provider is active.")
        if self.RefreshUI then self:RefreshUI() end
        return
    end
    if ENV_NAMES[key] and key ~= "DANDERSFRAMES" then
...
function ZD:GetCompatibilitySummary()
    ...
    return {
        ...
        "Detection provider: " .. tostring((self:GetDandersFramesIntegrationStatus() or {}).displayName or "Native Blizzard-managed"),
    }
end
...
if not ZD._listUIWrapped then
    D.ShowHidePriorityListUI = function()
        if ZD.CreateUI then
            local f = ZD:CreateUI(); f:Show(); ZD.listFocus = "priority"; ZD:ShowPage("lists")
        end
    end
    D.ShowHideSkipListUI = function()
        if ZD.CreateUI then
            local f = ZD:CreateUI(); f:Show(); ZD.listFocus = "skip"; ZD:ShowPage("lists")
        end
    end
